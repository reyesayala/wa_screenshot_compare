...
def create_with_csv(traces_in_path, csv_out_name):
    ...
    with open(csv_out_name, 'w+') as csv_file_out:
        ...
        csv_writer.writerow(["archive_id", "url_id", "network_requests_urls", "priority"])

        for filename in os.listdir(traces_in_path):
            file_components = re.split(r'[.]', filename) # Filename is structured as: archiveid_urlid.json
                ...
                for element in data['traceEvents']:
                    if element['cat'] == 'devtools.timeline' and\
                       ...
                       'url' in element['args']['data']:
                        ...
                        csv_writer.writerow([archive_id, url_id, url, priority])
            ...
            for element in data['traceEvents']:
                if element['cat'] == 'devtools.timeline' and\
                   ...
                   'url' in element['args']['data']:
                    ...
                    cursor.execute(
                            'INSERT INTO current_network_requests VALUES ({0}, {1}, "{2}", "{3}");'\
                                    .format(archive_id, url_id, url, priority))
...
def connect_sql(path):
    ...
    if cursor.fetchone()[0] == 1:
        cursor.execute("CREATE TABLE IF NOT EXISTS current_network_requests (archiveID INT, urlID INT, "
                       "networkURL TEXT, priority TEXT, FOREIGN KEY (archiveID) REFERENCES "
                       "collection_name(archiveID));")
        connection.commit()
    
        cursor.execute("SELECT DISTINCT archiveID FROM current_network_requests;")
        connection.commit()
