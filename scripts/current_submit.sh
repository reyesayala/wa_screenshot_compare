...
python3 "$scripts_path"/read_seed.py --csv="$current_seed_path" --out="$working_directory"/index/current_urls.csv --name="$archive_name" --ext=$extension --sort
python3 "$scripts_path"/current_screenshot.py --csv="$working_directory"/index/current_urls.csv --indexcsv="$working_directory"/index/current_index.csv --picsout="$working_directory"/pics_current --method=1
