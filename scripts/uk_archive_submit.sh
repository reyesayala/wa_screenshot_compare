...
# this is the bash script for the UK archive collection, make sure the path for wa_screenshot_compare is from the UKarchive repository
...
max_num_lines=$(( $(( $(( $( wc -l < "$archive_url_path" ) / 1000 )) + 1 )) * 1000 ))
...
do
    ...
    then
        python3 "$scripts_path"/archive_screenshot.py --csv="$archive_url_path" --indexcsv="$working_directory"/index/archive_index_$counter.csv --picsout="$working_directory"/pics_archive_$counter --method=1 --range=$(($counter - 999)),$counter --banner
    else
        python3 "$scripts_path"/archive_screenshot.py --csv="$archive_url_path" --indexcsv="$working_directory"/index/archive_index_$counter.csv --picsout="$working_directory"/pics_archive_$counter --method=1 --range=$(($counter - 999)),$counter --banner &
    fi
...
python3 "$scripts_path"/calculate_similarity.py --csv="$working_directory"/index/filenames.csv --currdir="$pics_current_path" --archdir="$working_directory"/pics_archive_combined --out="$working_directory"/index/scored.csv --ssim --mse --vec
