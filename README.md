...
> **Notes:** 
> *1. A valid seed csv file should have the current URLs as the first column, and the archive urls as the second column. Other columns are ignored in the program. The output csv file (file_names.csv) generated by get_file_names.py can be used as input csv in this case*
>*2. The output CSV file will have six columns, current_url, archive_url, current_title, archive_title, similarity_score, content_drift, and Note.*
>*3. threshold value is the value to determine whether the website pairs are content drift or not. The pairs with similarity score below the threshold value will be marked as content drift.*
>*4. The program uses selenium webdriver. One may needs to clean the tmp file created by the selenium if necessary.*
